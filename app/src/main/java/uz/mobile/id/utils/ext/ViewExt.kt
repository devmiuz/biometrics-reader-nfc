...
fun View.snackbar(@StringRes message: Int) = Snackbar.make(rootView, message, Snackbar.LENGTH_SHORT).show()

fun View.snackbar(message: String) = Snackbar.make(rootView, message, Snackbar.LENGTH_SHORT).show()
...
inline fun View.snack(
  resId: Int,
  f: Snackbar.() -> Unit, @IdRes length: Int = Snackbar.LENGTH_LONG
) {
...
fun ViewGroup.inflate(layoutRes: Int, attachRoot: Boolean = false) =
...
fun View.hideAnimWithScale(
  duration: Long = 200,
  withEndInvisible: Boolean = false,
  ...
  if (this.isVisible()) {
    this.animate()
      ...
      .withEndAction { if (withEndInvisible) this.invisible() else this.hide() }
      .start()
  }
}
...
fun View.showAnimWithFade(duration: Long = 200, endAction: () -> Unit = {}) {
//  if (isVisible().not()) {
    alpha = 0f
  show()
  animate()
...
fun View.hideAnimWithFade(duration: Long = 200, endAction: () -> Unit = {}) {
  if (isVisible()) {
    alpha = 1f
    animate()
      .alpha(0f)
...
fun LottieAnimationView.startLottieAnimation(action: () -> Unit) {
  playAnimation()
